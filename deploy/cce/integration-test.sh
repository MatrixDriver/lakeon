#!/usr/bin/env bash
#
# Lakeon Integration Tests — CCE Deployment Mode
#
# Tests single-tenant and multi-tenant scenarios against a live CCE cluster.
# Records compute-node startup time for each database creation.
#
# Differences from local mode:
#   - No port-forward — connects directly to ELB external IP
#   - Auto-detects LoadBalancer IP from lakeon-api Service
#
# Prerequisites:
#   - kubeconfig pointing to CCE cluster
#   - kubectl, curl, jq available
#   - lakeon-api Service type LoadBalancer with external IP assigned
#
# Usage:
#   ./deploy/cce/integration-test.sh
#   API_URL=http://<elb-ip>:8080 ./deploy/cce/integration-test.sh
...
# ─── Auto-detect API URL from LoadBalancer ───────────────────────────────────
detect_api_url() {
    if [[ -n "${API_URL:-}" ]]; then
    ...
    local api_port
    api_port=$(kubectl get svc lakeon-api -n "$NAMESPACE" -o jsonpath='{.spec.ports[0].port}' 2>/dev/null || echo "8080")

    local elb_ip=""
    local attempts=0
    echo "Detecting LoadBalancer external IP for lakeon-api..."

    while (( attempts < 30 )); do
        elb_ip=$(kubectl get svc lakeon-api -n "$NAMESPACE" \
            -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "")
        if [[ -n "$elb_ip" ]]; then
            API_URL="http://${elb_ip}:${api_port}"
            echo "Detected API_URL: ${API_URL}"
            return 0
        fi
        sleep 2
        attempts=$((attempts + 1))
    ...
    echo "ERROR: Could not detect LoadBalancer IP after 60s." >&2
    echo "Hint: Set API_URL manually: API_URL=http://<elb-ip>:8080 $0" >&2
    exit 1
}
...
main() {
    echo ""
    echo "═══════════════════════════════════════════════════════════════"
    echo "  Lakeon Integration Tests — CCE Deployment"
