...
```
              ...
              (端口 4432)             (端口 8080 / 80)
                    ...
                    ▼                     ▼
            ┌──────────────┐     ┌──────────────────┐     ┌──────────────┐
            │  Neon Proxy  │────▶│   Lakeon API     │◀────│ Web 控制台   │
            │  (2 副本)    │     │  (Spring Boot)   │     │ (Vue 3 SPA)  │
            └──────┬───────┘     │  (2 副本)        │     │ Nginx 反代   │
                   │             └────────┬─────────┘     └──────────────┘
                   │                      │
                   │              ┌───────┴────────┐
...
| api_key | VARCHAR(64) UNIQUE | 格式 `lk_` + 64位十六进制，SecureRandom 生成 |
...
```
lakeon 命名空间
├── Deployment: lakeon-api (2 副本)
├── Deployment: proxy (2 副本)
├── Deployment: pageserver (1 副本)
├── Deployment: storage-broker (1 副本)
├── StatefulSet: safekeeper (3 副本, Headless Service)
├── ConfigMap: lakeon-api-config
├── Secret: api-credentials (DB 密码, Proxy Token)
├── Secret: obs-credentials (OBS AK/SK)
├── Service: lakeon-api (ClusterIP :8080)
├── Service: proxy (ClusterIP :4432, :7000)
...
[LoadBalancer / Ingress]
    ...
    │                   (:55433)         (/proxy/*)
    │
    └── :8080 ──▶ API Service ──▶ Lakeon API Pods
                                       │
                                       ▼
                                  元数据 DB (:5432)
                                  Pageserver (:9898)
...
集群内部通信
    Compute Pod ──▶ Pageserver (:6400)
    Compute Pod ──▶ Safekeeper (:5454)
    Pageserver  ──▶ Storage Broker (:50051) ──▶ OBS
    Safekeeper  ──▶ Storage Broker (:50051) ──▶ OBS
```
...
Prometheus 每 15 秒采集以下目标：

| 目标 | 端点 | 关键指标 |
|------|------|----------|
| Lakeon API | :8080/actuator/prometheus | HTTP 请求延迟、计算唤醒失败数 |
| Pageserver | :9898/metrics | 页面缓存命中率、I/O 延迟 |
| Safekeeper | :7676/metrics | WAL 复制延迟、quorum 状态 |
| Proxy | :7000/metrics | 连接数、路由延迟 |

### 7.2 告警规则

| 告警名称 | 条件 | 级别 |
|----------|------|------|
| PageserverDown | Pageserver 不可达超过 1 分钟 | CRITICAL |
| SafekeeperDown | 少于 2 个 Safekeeper 运行 | CRITICAL |
| ComputeWakeupFailed | 5 分钟内有唤醒失败 | WARNING |
| HighStorageUsage | 存储使用超过 90% | WARNING |
| APIHighLatency | API P99 延迟超过 2 秒 | WARNING |

### 7.3 Grafana 仪表盘

预配置的 Lakeon Overview 仪表盘包含：
- 活跃数据库实例数（按状态分组）
- Compute 唤醒延迟分布
- 存储使用趋势
- API 请求 QPS 和延迟
