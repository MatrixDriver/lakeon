...
/**
 * Unit tests for {@link AgentFSService#append(TenantEntity, String, byte[], String)}.
 *
 ...
 *
 * Strategy: mock {@link AgentFSDatabaseManager} so every call to
 ...
 * context is not loaded — only the {@code AgentFSService} bean is constructed
 * by hand.
 */
@Testcontainers
...
class AppendIfMatchTest {
    ...
    void setup() throws SQLException {
        ...
        // AgentFSDatabaseManager#FILES_SCHEMA, minus the CDC trigger (not
        // exercised by append).
    ...
    void append_with_matching_etag_succeeds() {
        ...
        AgentFSService.FileRow first = svc.append(tenant, "/memory/notes.md", "hello".getBytes(), null);
        assertNotNull(first.etag, "initial append must produce an etag");
        assertEquals(5, first.size);

        // Re-append with the freshly-returned etag → must succeed.
        AgentFSService.FileRow second = svc.append(
    ...
    void append_with_stale_etag_throws_precondition_failed() {
        AgentFSService.FileRow seeded = svc.append(tenant, "/memory/notes.md", "hello".getBytes(), null);
        ...
        AgentFSService.FileRow after = svc.get(tenant, "/memory/notes.md");
    ...
    void append_with_null_ifmatch_is_legacy_path() {
        ...
        AgentFSService.FileRow created = svc.append(tenant, "/memory/notes.md", "abc".getBytes(), null);
        assertEquals(3, created.size);
        assertArrayEquals("abc".getBytes(), created.data);

        // A second null-ifMatch call still works, no precondition check.
        AgentFSService.FileRow extended = svc.append(tenant, "/memory/notes.md", "def".getBytes(), null);
    ...
    void append_with_empty_ifmatch_is_legacy_path() {
        ...
        AgentFSService.FileRow created = svc.append(tenant, "/memory/notes.md", "abc".getBytes(), "");
        assertEquals(3, created.size);

        AgentFSService.FileRow extended = svc.append(tenant, "/memory/notes.md", "def".getBytes(), "");
        assertEquals(6, extended.size);
    }
    ...
    void single_arg_overload_still_works() {
        ...
        AgentFSService.FileRow row = svc.append(tenant, "/memory/notes.md", "hello".getBytes());
