...
class LakebaseCdfIncrementalPollerTest {
    ...
    void pollRunningStreamsCommitsEventsAndDeletesOnlyAfterSuccessfulCommit() throws Exception {
        ...
        when(events.getObject(eq("created_at"), eq(OffsetDateTime.class)))
                .thenReturn(
                        OffsetDateTime.now().minus(Duration.ofMillis(900)),
                        OffsetDateTime.now().minus(Duration.ofMillis(500)));
        ...
        assertThat(stream.getLastCommitLsn()).isEqualTo("0/30");
        assertThat(stream.getLastSnapshotId()).isEqualTo(2L);
        assertThat(stream.getObservedLagMs()).isNotNull();
        assertThat(stream.getLastError()).isNull();
    ...
    @Test
    void failedCommitMarksStreamFailedAndPersistsLastErrorWithoutDeletingEvents() throws Exception {
        LakebaseCdfStreamRepository repository = mock(LakebaseCdfStreamRepository.class);
        LakebaseBranchConnectionProvider connectionProvider = mock(LakebaseBranchConnectionProvider.class);
        LakebaseCdfWorker worker = mock(LakebaseCdfWorker.class);
        LakebaseCdfIncrementalPoller poller = new LakebaseCdfIncrementalPoller(
        ...
        when(events.getLong("event_id")).thenReturn(41L);
        when(events.getString("op")).thenReturn("INSERT");
        when(events.getString("row_json")).thenReturn("{\"id\":101}");
        when(events.getObject(eq("created_at"), eq(OffsetDateTime.class)))
                .thenReturn(OffsetDateTime.now().minus(Duration.ofMillis(500)));
        when(connection.createStatement()).thenReturn(lsnStatement);
        when(lsnStatement.executeQuery("SELECT pg_current_wal_lsn()")).thenReturn(lsn);
        when(lsn.next()).thenReturn(true);
        when(lsn.getString(1)).thenReturn("0/30");
        doThrow(new com.lakeon.service.exception.BadRequestException("failed to commit CDF batch: writer failed"))
                .when(worker).commitBatch(eq(connection), eq(stream), any(CdfBatch.class));

        poller.pollRunningStreams();

        assertThat(stream.getStatus()).isEqualTo("FAILED");
        assertThat(stream.getLastError()).contains("writer failed");
        assertThat(stream.getLastCommitLsn()).isNull();
        assertThat(stream.getLastSnapshotId()).isNull();
        verify(deleteEvents, never()).executeUpdate();
        verify(repository).save(stream);
    }
