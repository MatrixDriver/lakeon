...
- **弹性算力**：1cu ~ 8cu 按需配置
- **AI 知识库**：文档上传、解析、切片、向量化、混合检索 (pgvector + tsvector)
- **Serverless 数据湖**：Python/Ray/微调任务，CCI Kata VM 安全隔离
...
| Web 控制台 | Vue 3 + TypeScript + TinyVue (港湾暖色调, ⌘K 命令面板) |
| SRE 运维控制台 | Vue 3 + TypeScript (港湾暖色调, 独立部署的 lakeon-admin) |
| 记忆服务 | Python FastAPI + pgvector (记忆存储/检索/摘要) |
| 成本监控 | 华为云 CBC 账单 API + 资源单价预估 |
| 可观测性 | Micrometer + Prometheus + 自建告警 + K8s Metrics API |
| 知识管线 | Python 3.12 + pymupdf4llm + psycopg2 + pgvector |
| 向量检索 | pgvector (HNSW) + tsvector (GIN) + RRF 融合 |
| Embedding | BGE-M3 自托管 (V100 GPU, OpenAI 兼容 API) |
| LLM 推理 | Qwen3.5-9B FP16 自托管 (V100 GPU, vLLM) |
| 重排序 | BGE-Reranker-v2-m3 (自建 FastAPI 服务) |
| 数据湖 | CCI Serverless (Kata VM) + KubeRay |
| 数据生产线 | Python FastAPI + Ray (DAG 编排引擎) |
| 日志系统 | Go log-collector + Fluent Bit + 结构化 JSON |
| MCP Server | 原生 HTTP MCP 内嵌 API + dbay-sre-mcp |

---

## 2. 整体架构

![Lakeon 架构图](dbay.drawio.svg)
...
```
               ...
               ┌───────────────┼───────────────┐
               │               │               │
         PostgreSQL 连接   REST API (HTTPS)  Web 控制台
         (端口 4432)       (端口 8443)      (dbay.cloud)
               │               │               │
               ▼               ▼               ▼
       ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
       │  Neon Proxy  │─│  Lakeon API  │◀│ Web 控制台   │ │ SRE Admin    │
       │  (ELB:4432)  │▶│(HTTPS:8090)  │ │ (Railway)    │ │ (Railway)    │
       └──────┬───────┘ └──────┬───────┘ │浏览器直连 API │ │ Admin Token  │
              │                │         └──────────────┘ └──────┬───────┘
              │         ┌──────┼──────┐                          │
              │         │      │      │                          │
              ▼         ▼      ▼      ▼                          ▼
     ┌────────────┐ ┌──────┐┌─────┐┌──────────┐        ┌──────────────┐
     │ Compute Pod│ │ K8s  ││ RDS ││Orchestr. │        │  Lakeon API  │
     │ (按需创建)  │ │ API  ││(VPC)││(FastAPI)  │        │  /admin/*    │
     │ PG17:55433 │ └──────┘└─────┘└──────────┘        └──────────────┘
     └──────┬─────┘
            │    ┌──────────────────────────────────┐
            │    │  GPU 节点 (V100)                  │
            │    │  embedding-svc (BGE-M3)           │
            │    │  llm-svc (Qwen3.5-9B vLLM)       │
            │    └──────────────────────────────────┘
            ▼
     ┌────────────────┐
     │  Pageserver    │
...
| 组件 | 职责 | 副本数 | 端口 |
|------|------|--------|------|
| **Lakeon API** | 管控面，数据库/分支 CRUD，Compute 生命周期管理，Admin API | 1 | 8090 (HTTPS, hostNetwork) |
...
| **Web 控制台** | 用户自助管理界面（DBay 品牌），Railway 托管，浏览器直连 API | Railway | 443 (HTTPS) |
| **SRE 运维控制台** | 管理员监控和运维界面，Admin Token 认证，Railway 托管 | Railway | 443 (HTTPS) |
| **Memory Service** | 记忆存储/检索/摘要服务 (FastAPI)，per-tenant 隔离 | CCE | 8100 (HTTP) |
| **Orchestrator** | 数据生产线 DAG 编排引擎 (FastAPI + Ray) | CCE | 8200 (HTTP) |
| **Embedding Service** | BGE-M3 向量化服务，OpenAI 兼容 `/v1/embeddings` | CCE (V100 GPU) | 8000 (HTTP) |
| **LLM Service** | Qwen3.5-9B 推理服务 (vLLM)，摘要/重写/AI SQL | CCE (V100 GPU) | 8000 (HTTP) |
| **Log Collector** | 日志收集服务 (Go)，HTTP 接收 + PG 批量写入 | CCE | 8080 (HTTP) |
| **Fluent Bit** | 容器日志采集 DaemonSet，CRI parser → log-collector | CCE (DaemonSet) | — |
...
文档上传 → OBS → Knowledge Job Pod → 用户 Compute Pod (PG)

```
用户 ──POST /knowledge-bases/{kbId}/documents──▶ Lakeon API
                                                     │
                                         1. 上传文件到 OBS
                                         2. 创建 DocumentEntity (UPLOADING)
                                         3. 启动 Knowledge Job Pod
                                                     │
                                                     ▼
                                           Knowledge Job Pod (CCI)
                                                     │
                                         1. 从 OBS 下载文档
                                         2. 解析 (pymupdf4llm/docx/epub/xlsx/pptx)
                                         3. 切片 (structure-aware, overlap)
                                         4. 向量化 (内部 BGE-M3 GPU 服务)
                                         5. 写入用户 PG (pgvector)
                                         6. 文档摘要 (内部 Qwen3.5-9B)
                                                     │
                                                     ▼
                                           用户 Compute Pod (PG)
                                           knowledge_chunks 表
...
搜索采用 Hybrid Retrieval (RRF 融合):
- **语义搜索**: pgvector HNSW cosine similarity (top-20)
- **全文搜索**: tsvector GIN ts_rank_cd (top-20)
- **融合**: Reciprocal Rank Fusion: `1/(60+rank_semantic) + 1/(60+rank_fts)`
- **重排序** (可选): BGE-Reranker-v2-m3

### 2.4 数据湖架构

```
用户 ──POST /datalake/jobs──▶ Lakeon API (DatalakeController)
                                    │
                      ┌─────────────┼─────────────┐
                      │             │             │
                PythonJobRunner  RayJobRunner  FinetuneJobRunner
                      │             │             │
                      ▼             ▼             ▼
                   CCI K8s Job   CCI RayJob   CCI Ray Train
                   (Kata VM)     (Kata VM)    (Kata VM)
                      │             │             │
                      └─────────────┼─────────────┘
                                    │
                         DatalakeStatusPoller
                         (sync job status)
```

三类任务通过 Virtual Kubelet 调度到 CCI Serverless:
- **PYTHON**: 单 Pod K8s Job，适合 ETL、数据处理
- **RAY**: RayJob CRD，适合分布式计算
- **FINETUNE**: Ray Train 模板注入，适合模型微调 (SFT/DPO/GRPO)

### 2.5 自托管 GPU 推理架构

```
Lakeon API ──────────────▶ embedding-svc (V100 GPU)
  │  摘要/重写/AI SQL         BGE-M3, /v1/embeddings
  │                           OpenAI 兼容 API
  ▼
llm-svc (V100 GPU)
  Qwen3.5-9B FP16
...
所有 AI 服务通过可配置的 `internal.llm.*` 参数统一路由到内部 GPU 服务:
- **Embedding**: 知识库文档向量化、搜索查询向量化
- **LLM**: 文档摘要生成、查询重写、AI SQL 助手、SRE AI 诊断

模型通过 PVC 挂载加载，支持离线模式运行。
...
容器 stdout/stderr ──▶ Fluent Bit (DaemonSet)
                    ...
                    log-collector (Go)
                          │ HTTP 接收 + 批量写入
                          ▼
                    dbay-logs (PG, system 租户)
                          │
              ┌───────────┼───────────┐
              ▼           ▼           ▼
         SRE Console  dbay-sre-mcp  Admin API
         (日志诊断)   (AI 工具)     (查询端点)
```

- **结构化日志**: Java (RequestIdFilter + MDC) 和 Python (lakeon-log 模块) 统一 JSON 格式
- **全链路追踪**: requestId 从 API 透传到 Job Pod，支持 log_trace 跟踪
- **SRE MCP**: log_search/log_trace/log_errors/log_stats 工具，集成到 SRE AI 助手

### 2.7 数据生产线架构

```
Console DAG 编辑器 ──▶ Lakeon API (PipelineController)
                             │
                     Pipeline Version (DAG JSON)
                             │
                             ▼
                   ...
                   ┌─────────┼─────────┐
                   ▼                   ▼
            PythonJobRunner      RayJobRunner
            (单 Pod, CCI)       (RayJob CRD, CCI)
                   │                   │
                   └─────────┬─────────┘
                             ▼
                    Component 框架
...
- **双引擎**: 轻量任务用 Python 单 Pod，分布式任务用 Ray
- **DAG 调度**: 拓扑排序 + 状态机 (PENDING → RUNNING → COMPLETED)
...
| 字段 | 类型 | 说明 |
|------|------|------|
| id | VARCHAR(64) PK | 格式 `kb_xxxxxxxx` |
| tenant_id | VARCHAR(64) FK | 所属租户 |
| database_id | VARCHAR(64) FK | 关联数据库 |
| name | VARCHAR(255) | KB 名称 |
| type | VARCHAR(20) | DOCUMENT / TABLE |
| status | VARCHAR(20) | CREATING / READY / ERROR |
| embedding_model | VARCHAR(128) | 向量化模型 |

#### documents

| 字段 | 类型 | 说明 |
|------|------|------|
| id | VARCHAR(64) PK | 格式 `doc_xxxxxxxx` |
...
#### knowledge_chunks (用户 PG 内)

| 字段 | 类型 | 说明 |
|------|------|------|
| id | SERIAL PK | 自增 |
| document_id | VARCHAR(32) | 文档 ID |
| chunk_index | INT | 切片序号 |
| content | TEXT | 切片文本 |
| embedding | vector(1024) | BGE-M3 向量 |
| metadata | JSONB | 元数据 |

索引: HNSW (embedding vector_cosine_ops), GIN (to_tsvector('simple', content))
...
├── Deployment: lakeon-admin (Railway 托管, SRE 运维控制台)
├── Deployment: lakeon-console (Railway 托管, Web 控制台)
...
├── Secret: api-keystore (PKCS12 TLS 证书)
├── Service: lakeon-api (LoadBalancer, ELB:8080 → :8090, hostNetwork + HTTPS)
...
### 5.3 网络拓扑 (华为云 CCE + Railway)

```
用户浏览器 (中国国内)
    │
    ├── https://dbay.cloud ──────────▶ [Railway] ──▶ Web 控制台 (Vue 3 SPA)
    │                                                    │
    │   浏览器直接调用 API（绕过 Railway 代理，            │
    │   避免中国 ISP 拦截境外 POST 请求）                   ▼
    │                                          https://api.dbay.cloud:8443
    │                                                    │
    ├── https://api.dbay.cloud:8443 ─────────────────────┘
    │       │
    │       ▼
    │   [华为云 ELB (共享型, EIP: 114.116.210.49)]
    │       │
    │       ├── :8443 (TCP 透传) ──▶ 节点 :8090 ──▶ Lakeon API Pod (HTTPS, hostNetwork)
    │       │                                              │
    │       │                                     ┌────────┼────────┐
    │       │                                     ▼        ▼        ▼
    ...
    │       │                                :5432      :9898
    │       │
    │       ├── :4432 (TCP) ──▶ Proxy Service ──▶ Proxy Pod
    │       │                                        │
    │       │                                ┌───────┴───────┐
    │       │                                ▼               ▼
    ...
    │       │                         (:55433)         (/proxy/*)
    │       │
    │       └── :8080 (TCP) ──▶ API Service ──▶ Lakeon API Pod (内部/健康检查)
    │
    └── PostgreSQL 连接 ──▶ ELB:4432 ──▶ Proxy ──▶ Compute Pod
...
集群内部通信 (VPC 内网)
    ...
    Lakeon API  ──▶ RDS (:5432, hostNetwork 模式直连)
```

#### HTTPS 方案说明

共享型 ELB 的 443 端口被 ADM (Anti-DDoS Middleware) 设备拦截，无法用于 HTTPS 终结或 TCP 透传。
因此采用以下方案：

| 层次 | 方案 | 说明 |
|------|------|------|
| ELB 监听器 | TCP:8443 → 节点:8090 | 纯 TCP 透传，不做 TLS 终结 |
| 应用层 TLS | Spring Boot HTTPS (PKCS12) | Let's Encrypt RSA 证书，acme.sh 签发 |
| 证书挂载 | K8s Secret `api-keystore` | 包含 keystore.p12，挂载到 /app/secrets/ |
| 前端调用 | 浏览器直连 `https://api.dbay.cloud:8443` | 绕过 Railway 代理，避免 ISP 拦截 |
| DNS | api.dbay.cloud → 114.116.210.49 | 华为云 DNS，NS 委托自聚名 |
| 域名 | dbay.cloud (聚名注册) | console: dbay.cloud (Railway), API: api.dbay.cloud |
...
| 功能 | 说明 |
|------|------|
| **数据库管理** | 全局数据库列表（默认首页）、按状态/租户筛选、卡片/表格视图 |
