# AgentFS Mount 模式韧性增强设计（ETag 冲突检测 + 下行拉取）
...
不在范围：sync 模式（独立 spec `2026-04-23-agentfs-sync-vs-mount.md`）、3-way merge、占位文件、macOS File Provider。
...
|---|---|---|---|
| `/files/put` 接受 `if_match` | ✅ `AgentFSService.java:127` | ❌ uplink 不传 | ✅ `put_strict` 传 |
...
**A1**：`AgentFSService.append(...)` 增加 `String ifMatch` 参数，进入事务后比对 `existing.etag`，不匹配抛 `BadRequestException("precondition_failed")`。

**A2**：`AgentFSController.appendFile` 从 body 读 `if_match` 字段透传给 service。

**A3**：`AgentFSController.batch` 的每个 op item 支持可选 `if_match`（put / append / delete 三类），controller 解析后透传给对应 service 方法。**返回值结构扩展**：每个 op 的结果带 `etag`（新版本）、`status`（`ok` 或 `precondition_failed`）。
...
**A7**：`dbay_api.rs::agentfs_batch` 签名扩展返回 `Vec<BatchOpResult>`（含 status + etag），caller 才能 per-op 处理

### 3.4 冲突处理（客户端）

发生 412 时：
1. `agentfs_get(path)` 拉云端最新版本 + 新 etag
...
**E2E**（`tests/e2e/test_agentfs_etag_conflict.py`）
...
**E2E**（`tests/e2e/test_agentfs_pull.py`）
- 测试租户先通过 API 写 3 个文件
- 启动 dbay-fuse pull
...
**E2E 跨流程**（`tests/e2e/test_agentfs_mount_resume.py`）
...
| 文件 | 改动 |
|---|---|
| `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSService.java` | `append(...)` 增加 `ifMatch` 参数与比对 |
| `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java` | `appendFile` 解析 `if_match`；`batch` 支持 per-op if_match + per-op 结果含 etag/status |
...
| `dbay-fuse/src/dbay_api.rs` | `agentfs_batch` 返回 `Vec<BatchOpResult>`；`agentfs_get` 已存在；扩展 `agentfs_append` 支持 if_match 选项 |
...
| 文件 | 改动 |
|---|---|
| `docs/agentfs-user-guide.md` | §9.2 标 ✅；§9.3 标 ✅；§12 Phase 5 标 ✅；新增 §6.4 冲突文件如何处理 |
...
| `tests/e2e/test_agentfs_etag_conflict.py` | **新建** |
| `tests/e2e/test_agentfs_pull.py` | **新建** |
| `tests/e2e/test_agentfs_mount_resume.py` | **新建**（pull + mount 综合） |
...
- 冲突分支 + conflict log
- 跑 E2E test_agentfs_etag_conflict

**Phase 3 — pull 子命令**
- pull.rs + main.rs Cmd::Pull
- E2E test_agentfs_pull

**Phase 4 — mount/takeover 集成 pull**
- main.rs Mount 默认 pull
- takeover.rs 调 pull
- E2E test_agentfs_mount_resume
...
- [ ] E2E `test_agentfs_etag_conflict`、`test_agentfs_pull`、`test_agentfs_mount_resume` 全绿
...
- `AgentFSService.append`：`lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSService.java:157`
- `AgentFSController.batch`：`lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java:138`
...
- `dbay_api::agentfs_put_strict`（已存在的参考实现）：`dbay-fuse/src/dbay_api.rs:428`
