...
GET  /api/v1/knowledge/bases/{kbId}/chunks/{chunkIndex}?doc_id={docId}
     → 单个切片详情（含 content, metadata, offsets）

GET  /api/v1/knowledge/bases/{kbId}/chunks/{chunkIndex}/context?doc_id={docId}
...
```
PUT    /api/v1/knowledge/bases/{kbId}/chunks/{chunkIndex}?doc_id={docId}
       → 编辑切片 content，即时重新生成 embedding，更新 char_count
       → 设置 edited=true, updated_at=now()

DELETE /api/v1/knowledge/bases/{kbId}/chunks/{chunkIndex}?doc_id={docId}
       → 删除切片，更新 document 的 chunks_count

POST   /api/v1/knowledge/bases/{kbId}/documents/{docId}/chunks
       → 新增切片（指定 content, chunk_index），生成 embedding
       → 使用浮点 chunk_index 插入（如在 #3 和 #4 之间插入 #3.5），避免大量 reindex
       → 设置 edited=true
```
...
- 不需要定时清理任务，上界确定，存储可控
- 并发保护：`DocumentEntity` 上加 `rechunk_status` 字段（IDLE / IN_PROGRESS），使用乐观锁。rechunk 开始时设为 IN_PROGRESS，完成/失败时设回 IDLE
