...
AgentFS 当前只提供一种客户端接入方式——**FUSE mount**（`dbay-fuse/src/main.rs`，使用 Rust `fuser 0.14` crate）。用户执行 `dbay-fuse mount --agent=<name>` 后，在 `~/.dbay/mnt/<agent>/` 得到一个挂载点，所有 agent 读写都经 FUSE 转发到 `/api/v1/agentfs/files/*`。
...
参考 Google Drive / Dropbox / OneDrive 的"本地目录同步"模式，我们想**增加一种不依赖 FUSE 的接入方式**：用户指定一个本地目录，dbay daemon 双向同步到 AgentFS 后端。
...
agent 进程 ─read/write─▶ ~/.dbay/mnt/foo/x.txt
                       ...
                       dbay-fuse (fuser crate)
                            │
                            ▼
              HTTP ─▶ /api/v1/agentfs/files/put
                      (AgentFSController.java:86)
                            │
                            ▼
                   PostgreSQL files 表
...
agent 进程 ─read/write─▶ ~/dbay-workspace/foo/x.txt   (普通 POSIX 文件)
         ...
         (etag/mtime/state)       (debounce + coalesce)
                                       │
                                       ▼
                         HTTP ─▶ /api/v1/agentfs/files/batch
                                  (已有端点, if_match 支持)
                                       │
                 ◀──── 反向同步 ◀──────┤
                                       │
                         SSE/poll ─▶ /api/v1/agentfs/events
                                      (新端点，消费 agentfs_events 表)
```

- 文件在本地是**普通文件**，所有工具（IDE、git、grep、rsync）直接可用
- daemon 监听文件系统事件 → debounce 合并 → 批量 PUT
- 反向：订阅 `agentfs_events` CDC 流 → 拉文件 → 写本地
- 离线可工作，上线后增量同步

**关键观察**：后端 API (`AgentFSController`) 完全不用改——PUT/GET/batch/list 已经全了，etag + if_match 语义也齐。改动集中在：新增 events 订阅端点 + 新 daemon。
...
- **超大单文件流式处理**（>50GB 单文件顺序扫描）：两种都不理想，mount 会 RTT 爆炸，sync 首次同步太慢。应该走对象存储直读（OBS SDK），不经 AgentFS。
- **极低延迟写**（<10ms 每次写）：两种都不行，应该用内存 KV 或本地 SQLite，AgentFS 不是设计目标。
...
- **事件订阅端点**：`GET /api/v1/agentfs/events?since=<id>&agent=<name>`（新加，SSE）

不改：
- AgentFS 后端文件操作 API（复用 `/files/put`、`/files/batch`、`/files/head`、`/list`、`/rename`、`/delete`）
...
CREATE TABLE sync_files (
  ...
  remote_path      TEXT NOT NULL,        -- AgentFS 路径
...
1. daemon 订阅 `GET /api/v1/agentfs/events?since=<cursor>&agent=<name>`（SSE）
...
**后端改动**：需要新增 `GET /api/v1/agentfs/events` 端点，消费现有 `agentfs_events` 表（`AgentFSDatabaseManager.java:60-72` 已有 CDC 触发器，直接读即可）。SSE 或 long-poll 均可，推荐 SSE。
...
当前后端已经支持：
- PUT 带 `if_match=<etag>` → 412 if 不匹配（`AgentFSController.java:86-87`）
- HEAD 返回 `etag + mtime_ns + size`
- idempotency：相同 etag 的 PUT 不更新 mtime（bb2d052b commit + `AgentFSService.java:324-327`）
...
- 批量上传走 `/api/v1/agentfs/files/batch`（已存在的批量端点，一次最多 100 op）
...
- 红军：Dropbox 的驱动是"家庭照片 10TB 本地塞不下"，是 consumer 大数据量场景。AgentFS 是 agent 工作目录，GB 级，需求不一样。Phase 1 不做占位文件也 OK。

**B2: "git 仓库同步必翻车，.git/index 天天冲突。"**
- 红军：默认忽略 `.git/` 是标配。VS Code 的远程开发、Dropbox 的建议文档也都这样要求。如果用户真要同步 git 历史，走 `git push` 到 AgentFS 暴露的 git 远端更干净。
...
**B6: "后端 `agentfs_events` 表会不会被 sync 订阅打爆？"**
...
- 后端：**新增 `GET /api/v1/agentfs/events` SSE 端点**（小改动）
...
4. 后端新增 `GET /api/v1/agentfs/events` SSE 端点，由谁负责？
...
- 后端 Controller：`lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java`
- 后端 Service（etag + if_match + 幂等 upsert）：`lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSService.java:324`
- 数据库 schema + CDC 触发器：`lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSDatabaseManager.java:45`
- Rust FUSE 客户端（可复用 HTTP client）：`dbay-fuse/src/main.rs`、`dbay-fuse/src/uplink_worker.rs`
- 幂等性回归测试：`tests/e2e/test_agentfs_idempotent.py`
