# AgentFS Mount Resilience Implementation Plan
...
**Spec reference:** `docs/superpowers/specs/2026-05-19-agentfs-mount-resilience-design.md`
...
- `tests/e2e/test_agentfs_etag_conflict.py` — cross-device 412 scenario
- `tests/e2e/test_agentfs_pull.py` — pull subcommand happy + conflict paths
- `tests/e2e/test_agentfs_mount_resume.py` — fresh-state mount picks up remote files
- `lakeon-api/src/test/java/com/lakeon/agentfs/AppendIfMatchTest.java` — unit test for append if_match

### Modified files
- `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSService.java` — `append(...)` gets `ifMatch` parameter
- `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java` — `appendFile` reads `if_match`; `batch` reads `if_match` for append; `batch` catches BadRequestException per-op to return `status:"precondition_failed"`
- `dbay-fuse/Cargo.toml` — add `rusqlite` with bundled feature
- `dbay-fuse/src/dbay_api.rs` — `agentfs_batch` returns `Vec<BatchOpResult>`; `agentfs_append` gains `if_match`-aware variant
...
- `docs/agentfs-user-guide.md` — update §9.2/§9.3/§12 and document conflict files

---

## Phase 1 — Server-side `if_match` + client ledger (no behavior change yet)

### Task 1: `AgentFSService.append` accepts `ifMatch`

**Files:**
- Modify: `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSService.java:157-184`
- Test: `lakeon-api/src/test/java/com/lakeon/agentfs/AppendIfMatchTest.java` (create)

- [ ] **Step 1: Write the failing test**

```java
// lakeon-api/src/test/java/com/lakeon/agentfs/AppendIfMatchTest.java
...
class AppendIfMatchTest {
    ...
    @Autowired AgentFSService svc;
    @Autowired AgentFSDatabaseManager dbm;

    private TenantEntity tenant;

    @BeforeEach
    void setUp() {
        tenant = AgentFSTestHelper.provisionScratchTenant(dbm);
    }

    @Test
    void append_with_matching_if_match_succeeds() {
        AgentFSService.FileRow base = svc.put(tenant, "/a.log", "hello".getBytes(),
                null, null, null);
        AgentFSService.FileRow after = svc.append(tenant, "/a.log",
    ...
    void append_with_null_if_match_still_works_legacy() {
        AgentFSService.FileRow base = svc.put(tenant, "/c.log", "hi".getBytes(),
                null, null, null);
        AgentFSService.FileRow after = svc.append(tenant, "/c.log", "!".getBytes(), null);
        assertThat(after.size).isEqualTo(3L);
    }
}
```

If `AgentFSTestHelper` doesn't exist, use a minimal inline replacement: look at existing `lakeon-api/src/test/java/com/lakeon/agentfs/PathWhitelistTest.java` for how tests construct tenants.
...
- [ ] **Step 3: Modify `AgentFSService.append`**

Open `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSService.java`. Replace the existing `append` method (lines 157–184) with:
...
```bash
git add lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSService.java \
        lakeon-api/src/test/java/com/lakeon/agentfs/AppendIfMatchTest.java
git commit -m "feat(agentfs): AgentFSService.append accepts ifMatch (precondition_failed on mismatch)"
```
...
**Files:**
- Modify: `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java:93-100` (appendFile)
- Modify: `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java:168-173` (batch append case)

- [ ] **Step 1: Write the failing test**

Add to `lakeon-api/src/test/java/com/lakeon/agentfs/AppendIfMatchTest.java`:
...
void controller_appendFile_passes_if_match_through() throws Exception {
    ...
    AgentFSService.FileRow base = svc.put(tenant, "/d.log", "hi".getBytes(),
            null, null, null);
    String body = om.writeValueAsString(Map.of(
            "path", "/d.log",
            "data_base64", java.util.Base64.getEncoder().encodeToString("!".getBytes()),
            "if_match", "wrong"));
    mockMvc.perform(post("/api/v1/agentfs/files/append")
            .header("Authorization", "Bearer " + AgentFSTestHelper.apiKeyFor(tenant))
...
Open `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java`. Replace `appendFile` (lines 93–100):
...
```bash
git add lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java \
        lakeon-api/src/test/java/com/lakeon/agentfs/AppendIfMatchTest.java
git commit -m "feat(agentfs): controller wires if_match into /files/append + batch append"
```
...
**Files:**
- Modify: `lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java:138-195`
...
void batch_one_precondition_does_not_abort_other_ops() throws Exception {
    AgentFSService.FileRow a = svc.put(tenant, "/m.txt", "1".getBytes(), null, null, null);
    AgentFSService.FileRow b = svc.put(tenant, "/n.txt", "1".getBytes(), null, null, null);
    ...
    mockMvc.perform(post("/api/v1/agentfs/batch")
            .header("Authorization", "Bearer " + AgentFSTestHelper.apiKeyFor(tenant))
            .contentType("application/json").content(body))
...
In `AgentFSController.java`, wrap each case body inside the for-loop. Easiest: extract a helper closure. The simplest patch is to wrap the entire `switch` in a try/catch:
...
git add lakeon-api/src/main/java/com/lakeon/agentfs/AgentFSController.java \
        lakeon-api/src/test/java/com/lakeon/agentfs/AppendIfMatchTest.java
git commit -m "feat(agentfs): batch returns per-op precondition_failed instead of aborting"
```
...
### Task 6: `agentfs_batch` returns `Vec<BatchOpResult>`

The current `agentfs_batch` discards the response body. To populate the ledger we need each op's new etag.
...
(`agentfs_batch` is internal — verify behavior by checking `dbay-fuse` builds and a smoke test in next task.)

- [ ] **Step 2: Modify `agentfs_batch`**
...
In `dbay-fuse/src/uplink_worker.rs`, replace the `send_batch` function. After `cli.agentfs_batch(ops)?;`, accept the Vec but ignore it for now:

```rust
// Inside send_batch, replace `cli.agentfs_batch(ops)?;` line:
...
git commit -m "refactor(dbay-fuse): agentfs_batch returns Vec<BatchOpResult>"
```
...
fn handle_conflict(cli: &DbayClient, ledger: &Ledger, path: &str) -> Result<()> {
    let (remote_bytes, remote_etag, _mtime) = cli.agentfs_get(path)
...
### Task 10: E2E `test_agentfs_etag_conflict.py`

**Files:**
- Create: `tests/e2e/test_agentfs_etag_conflict.py`
...
def test_batch_one_precondition_fails_others_succeed(e2e_client):
    ...
    # snapshot both etags
    r_a = requests.get(f"{endpoint}/api/v1/agentfs/files/head",
                       params={"path": base64.urlsafe_b64encode(b"/batch-a.txt").rstrip(b"=").decode()},
                       headers={"Authorization": f"Bearer {key}"},
                       verify=False, timeout=30)
    r_b = requests.get(f"{endpoint}/api/v1/agentfs/files/head",
...
python3 -m pytest tests/e2e/test_agentfs_etag_conflict.py -v
```
...
```bash
git add tests/e2e/test_agentfs_etag_conflict.py
git commit -m "test(agentfs): E2E for if_match precondition_failed (put / append / batch)"
```
...
pub fn pull(
    ...
    let entries = cli.agentfs_list(prefix, true)
        .context("list remote")?;
...
fn download_and_write(
    ...
    let (bytes, etag, _mtime) = cli.agentfs_get(&remote.path)
...
### Task 13: E2E `test_agentfs_pull.py`

**Files:**
- Create: `tests/e2e/test_agentfs_pull.py`
...
def _put(endpoint, key, path, data, retries=10, delay=4):
    for _ in range(retries):
        r = requests.post(f"{endpoint}/api/v1/agentfs/files/put",
...
python3 -m pytest tests/e2e/test_agentfs_pull.py -v
```
...
```bash
git add tests/e2e/test_agentfs_pull.py
git commit -m "test(agentfs): E2E for dbay-fuse pull (download, skip, conflict sidecar)"
```
...
### Task 16: E2E `test_agentfs_mount_resume.py`

**Files:**
- Create: `tests/e2e/test_agentfs_mount_resume.py`
...
```bash
cd /Users/jacky/code/lakeon
python3 -m pytest tests/e2e/test_agentfs_mount_resume.py -v
```
...
```bash
git add tests/e2e/test_agentfs_mount_resume.py
git commit -m "test(agentfs): E2E mount-resume — pull onto empty state dir"
```
...
### Task 17: Update `docs/agentfs-user-guide.md`

**Files:**
- Modify: `docs/agentfs-user-guide.md` §9 / §12
...
```bash
git add docs/agentfs-user-guide.md
git commit -m "docs(agentfs): update mount-resilience status (if_match, pull, conflict files)"
```
...
**Spec coverage check (against `2026-05-19-agentfs-mount-resilience-design.md`):**
...
- §3.3 A7 agentfs_batch returns BatchOpResult → Task 6 ✓
