# AgentFS Phase 2 Rollout Runbook
...
- `swr.cn-north-4.myhuaweicloud.com/flex/lakeon-memory:0.2.7` — memory-svc with `POST /agentfs/derive`, `ingest_idempotent`, `delete_by_source_path`
...
```bash
# A4: install agentfs_events table + trigger on every agentfs_<uuid> DB
...
```bash
kubectl -n lakeon logs -l app=lakeon-api --tail=200 -f | grep -iE "agentfs|forwarder"
```
...
- `forwarder tenant=... target base provisioning; will retry` (first-time auto-provision of `agentfs-claude` base)

Watch for **ERROR** lines — any `memory-svc status=` failures mean /agentfs/derive isn't reachable; check `kubectl -n lakeon get svc memory-svc`.

## Step F4 — run full E2E suite

```bash
cd /Users/jacky/code/lakeon
python3 -m pytest tests/e2e/test_agentfs_phase2.py          -v  # 6 mechanism tests
python3 -m pytest tests/e2e/test_agentfs_phase2_quality.py  -v  # 10 quality tests (real corpus)
python3 -m pytest tests/e2e/test_derive_idempotent.py       -v  # 2 idempotency tests
python3 -m pytest tests/e2e/test_agentfs_idempotent.py      -v  # 2 server PUT idempotency (regression)
```
...
- "AgentFS 目标" radio column visible
- `agentfs-claude` base (auto-created for your tenant) shows `[auto]` badge
- If target not yet set, banner at top "AgentFS 有 N 条待派生 memory..." appears

## Post-deploy smoke

1. Stats delta: `curl .../agentfs/stats` now returns derived memories count > 0 within ~1 minute of first forwarder cycle.
...
Forwarder can be disabled by setting `lakeon.agentfs.forwarder.enabled=false` in values.yaml (requires template support — currently ConditionalOnProperty passes directly to pod env; set in `deploy/helm/lakeon/templates/api-deployment.yaml` env section if needed).
