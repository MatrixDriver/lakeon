...
describe('ConsoleLayout', () => {
  ...
  it('narrows the console rail to Lakebase data, FS, and operations', async () => {
    ...
    expect(wrapper.find('.sidebar-nav').text()).toContain('Lakebase')
    expect(wrapper.findAll('.nav-item').map((node) => node.text())).toContain('数据库')
    expect(wrapper.find('.sidebar-nav').text()).not.toContain('数据总览')
    expect(wrapper.find('.sidebar-nav').text()).toContain('实时表同步')
