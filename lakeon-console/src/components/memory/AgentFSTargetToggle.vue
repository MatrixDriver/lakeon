...
import { setAgentFSMemoryTarget } from '@/api/agentfs'
...
async function setTarget() {
  ...
  try {
    await setAgentFSMemoryTarget(props.baseId)
    emit('changed', props.baseId)
  } catch (err) {
    console.error('set AgentFS target failed', err)
    emit('error', err)
  } finally {
...
<template>
  <label class="target-radio" :class="{ active: isActive, disabled: submitting }" @click.stop>
    <input
      ...
      @change="setTarget"
    />
    <span class="target-label">AgentFS 目标</span>
  </label>
</template>
