...
test.describe('Agent state workbench', () => {
  ...
  test('switches from database console to agent workbench', async ({ page }) => {
    ...
    await expect(page.locator('.sidebar-nav .side-title')).toContainText('智能体工作台')
    ...
    await expect(page.locator('.sidebar-nav').getByRole('link', { name: '证据' })).toBeVisible()
    ...
    await expect(page.locator('.detail-panel').getByText('All 100 quicksort tests passed.')).toBeVisible()
    ...
    await page.locator('.sidebar-nav').getByRole('link', { name: '证据' }).click()
